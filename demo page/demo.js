...
function refresh(){
 ...
 byId('incarcerationType').disabled=!incarcerated;
 if(!incarcerated)byId('incarcerationType').value='SEL';
...
document.addEventListener('click',e=>{
 ...
 if(button){
  ...
  if(action.includes('save')){
   const data=[...form.querySelectorAll('input,select,textarea')].map(el=>({id:el.id,value:el.value,checked:el.checked}));
